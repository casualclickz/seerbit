...
public class SeerbitTransactionControllerStatisticsTest {
	...
	@Test
	public void seerbitTransactionControllerTwoZeroOneTest() throws Exception {
		...
		mockMvc.perform(post("/transaction").contentType(APPLICATION_JSON_UTF8).content(requestJson)).andDo(print()).andExpect(status().isCreated());
		
		transactionDto = new LinkedHashMap<>();
		transactionDto.put("amount","15.4002");
		transactionDto.put("timestamp",seerbitTransactionUtils.formatLocalDateTimeToISODATETIMEFormat(LocalDateTime.now()));		
		requestJson = this.ow.writeValueAsString(transactionDto);		
		mockMvc.perform(post("/transaction").contentType(APPLICATION_JSON_UTF8).content(requestJson)).andDo(print()).andExpect(status().isCreated());
		
		transactionDto = new LinkedHashMap<>();
		transactionDto.put("amount","10.4322");
		transactionDto.put("timestamp",seerbitTransactionUtils.formatLocalDateTimeToISODATETIMEFormat(LocalDateTime.now()));		
		requestJson = this.ow.writeValueAsString(transactionDto);		
		mockMvc.perform(post("/transaction").contentType(APPLICATION_JSON_UTF8).content(requestJson)).andDo(print()).andExpect(status().isCreated());
		
		mockMvc.perform(get("/transaction").contentType(APPLICATION_JSON_UTF8)).andDo(print()).andExpect(status().isOk());
	}
}