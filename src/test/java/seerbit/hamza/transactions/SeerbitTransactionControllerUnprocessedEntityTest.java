...
public class SeerbitTransactionControllerUnprocessedEntityTest {
	...
	@Test
	public void seerbitTransactionControllerUnprocessedEntityTest() throws Exception {
		
		Map<String, String> transactionDto = new LinkedHashMap<>();
		transactionDto.put("amount","12.3343");
		transactionDto.put("timestamp",seerbitTransactionUtils.formatLocalDateTimeToISODATETIMEFormat(LocalDateTime.now().plusHours(1)));
		
		String requestJson = this.ow.writeValueAsString(transactionDto);
		
		mockMvc.perform(post("/transaction").contentType(APPLICATION_JSON_UTF8).content(requestJson)).andDo(print()).andExpect(status().isUnprocessableEntity());
	}
}